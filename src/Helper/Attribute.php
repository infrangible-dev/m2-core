...
class Attribute
{
    ...
    public function getAttributeId(AdapterInterface $dbAdapter, string $entityTypeCode, string $attributeCode): ?int
    {
        ...
        $attributeQuery = $dbAdapter->select()->from(
    ...
    public function getAttributeValues(
        ...
        $attributeCodeCompare = strcasecmp(
            $entityIdField,
            $attributeCode
        );

        $isEntityIdField = $attributeCodeCompare === 0;
                            ...
                            foreach ($source->getAllOptions() as $option) {
                                if (array_key_exists(
                                    ...
                                    if (array_key_exists(
                                        ...
                                        $attributeValueCompare = strcasecmp(
                                            strval($option[ 'value' ]),
                                            strval($attributeValue)
                                        );

                                        if ($attributeValueCompare === 0) {
